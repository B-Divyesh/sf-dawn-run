...
function privacy() {
  return shell(`<article class="legal"><h1 tabindex="-1">Privacy at Dawn Run</h1><p>Dawn Run stores your current run, settings, player code, and eight recent results in this browser.</p><p>Nothing is published until you choose Publish verified score. Publication sends your nickname, date, tool, score, reported time, and move record to Dawn Run.</p><p>Scores and move records are checked. Reported time does not change rank.</p><p>Published entries use your chosen nickname and expire after seven days. Dawn Run does not use analytics, advertising, accounts, or third-party scripts.</p><p>The demo uses separate keys that start with <code>demo:</code>. Reset demo and Start for real remove those keys. Demo submissions are not stored.</p><p><a href="/" data-nav>Return to the daily run</a>.</p></article>`);
}
